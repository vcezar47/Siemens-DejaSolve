...
* **Stribeck stiction**, already in `dejasolve/model.py`. Each shaft has more than one
...
configuration that a real machine settled into.
    ...
    python -m experiments.coupled --selftest    # the analytic Jacobian
    python -m experiments.coupled               # the comparison that matters
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=Path("results/coupled_results.json"))
    args = ap.parse_args()
