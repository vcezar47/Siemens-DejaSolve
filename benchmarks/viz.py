"""Projection data for the 3D view -- the archive, a matched pair, and the race.
    ...
    python -m benchmarks.viz            # writes viz_results.json
...
the same archive `benchmarks/bench.py` benchmarks, and the iteration counts it reports are
...
OUT = Path("results/viz_results.json")
RESULTS = Path("results/results.json")
ARCHIVE = Path("data/archive/cases.jsonl")
FAILURES = Path("data/archive/failures.jsonl")
...
def run(archive_path: Path = ARCHIVE, results_path: Path = RESULTS,
    ...
    if not results_path.exists():
        raise SystemExit(f"no {results_path} -- run `python -m benchmarks.bench` first")
