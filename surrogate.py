...
**Separate file, separate results, on purpose.** `bench.py` owns the headline
...
follows `fold.py` instead: its own experiment, its own JSON, quotable on its own.
...
mechanism, and safety comes from the gate on the *answer*.
    ...
    python surrogate.py            # the base circuit: what a prediction is worth
    python surrogate.py --fold     # the fold circuit: what it costs unverified
"""
...
import model
import verifier
from bench import PRESSURE_IDX, SPEED_IDX, load_archive, nearest
...
def _arm(entries: list[dict], key: str) -> dict:
    """Summarise one starting-guess arm the way bench.py reports its three."""
...
def main() -> int:
    ...
    ap.add_argument("--seed", type=int, default=99,
                    help="must match bench.py for the arms to be comparable")
    ap.add_argument("--out", type=Path, default=Path("surrogate_results.json"))
    ...
    ap.add_argument("--fold-out", type=Path,
                    default=Path("surrogate_fold_results.json"))
