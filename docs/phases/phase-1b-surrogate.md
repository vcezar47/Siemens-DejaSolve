...
Reproduce with `python -m benchmarks.surrogate`, or as *Phase 1b* inside `python run_all.py`.
...
**Separate file, separate results, on purpose.** `benchmarks/bench.py` owns the headline and `summary_hash` enumerates
its three arms by name, so a fourth arm there would move `830da3e6a4480676` and every document quoting it.
This follows the `benchmarks/fold.py` precedent instead: own experiment, own JSON. **The phase-1 hash is unchanged.**

`benchmarks/surrogate.py` re-solves the cold, nominal and warm arms itself and then cross-checks them case by case
...
`python -m benchmarks.surrogate --fold`. The question §6e left open: on the base circuit, gating the *prediction* fires 15
...
Same archive seed (11) and same 200 queries (seed 77) as `benchmarks/fold.py`, so the two experiments sit beside each
...
The `post_only` arm is in `benchmarks/surrogate.py` for exactly this reason: without it, "we check the prediction before
...
- **scikit-learn / XGBoost / an MLP.** More accuracy was the wrong axis. The brief was a stand-in that is fast
  ...
  architecture.
- **A fourth arm inside `benchmarks/bench.py`.** Would have moved the phase-1 summary hash for a result that is not the
  headline. The `benchmarks/fold.py` pattern exists for exactly this.
