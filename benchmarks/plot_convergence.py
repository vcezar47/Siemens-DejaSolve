"""The Phase 1 gate figure, drawn from results.json — nothing is hard-coded.
    ...
    python -m dejasolve.sweep && python -m benchmarks.bench && python -m benchmarks.plot_convergence
...
def main() -> None:
    ...
    ap.add_argument("--out", type=Path, default=Path("results/figs/convergence.png"))
    args = ap.parse_args()
    if not args.results.exists():
        raise SystemExit(f"no {args.results} -- run `python -m benchmarks.bench` first")
