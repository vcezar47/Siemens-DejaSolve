...
leaves alone: the physics rules in `verifier.py` never touch the setup vector,
...
def main() -> int:
    ...
    ap.add_argument("--seed", type=int, default=99,
                    help="must match bench.py so the reference arms line up")
    ap.add_argument("--out", type=Path, default=Path("dimensionality_results.json"))
    ap.add_argument("--fig", type=Path, default=Path("figs/dimensionality.png"))
    args = ap.parse_args()
