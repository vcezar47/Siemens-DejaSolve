...
Reproduce with `python -m benchmarks.failure_zone`, or as *Phase 2b* inside `python run_all.py`.
...
is hard, and reducing it to a tally throws that away. `dejasolve/sweep.py` was collecting failures and discarding them
...
- `dejasolve/sweep.py` writes `data/archive/failures.jsonl` alongside `cases.jsonl` — a separate file, because everything
...
- `benchmarks/failure_zone.py` tests the obvious candidate rule, in the same normalised setup space retrieval already
  uses:
...
Two further reasons it stays out of `dejasolve/verifier.py` for now:
...
- **Reporting the base-circuit result as evidence.** 5 failures is below the threshold for a conclusion;
  `benchmarks/failure_zone.py` carries `MIN_FAILURES = 30` and prints `UNDERPOWERED` rather than letting a reader draw
  one anyway.
