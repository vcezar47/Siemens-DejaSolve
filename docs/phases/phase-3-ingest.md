...
**Gate:** an end-to-end run from a messy log file → ✅ `python -m dejasolve --all`

## What was built

| file | role |
|---|---|
| [`../casecard.py`](../../dejasolve/casecard.py) | The Case Card: canonical units, provenance, explicit absence, plausibility check |
| [`../ingest.py`](../../dejasolve/ingest.py) | Layer 1, two backends (`rules`, `llm`) behind one interface, plus the accuracy measurement |
| [`../make_logs.py`](../../benchmarks/make_logs.py) | Five run artifacts with exact ground truth |
| [`../dejasolve.py`](../../dejasolve/pipeline.py) | The whole system in one path: artifact → Case Card → retrieval → verifier → warm solve → admissibility |
...
`dejasolve/pipeline.py` stops the run at three different layers, and the distinction is
the product:
...
`python -m dejasolve.ingest --compare`. Until that runs, **the LLM numbers do not exist and
must not be quoted.**
...
`dejasolve/pipeline.py` previously printed as it computed, so a service would have had to
...
- `run_all.py` now regenerates the fixtures too, so a clean machine has `logs/`
  before `dejasolve/pipeline.py --all` is run.
...
`dejasolve/pipeline.py` and the web headline report all three:
...
seventh fixture, `data/logs/part-bracket.log`, is an NX Nastran solver log with a bulk-data echo — a real artifact
...
**Re-verified 20 Aug, after these changes.** `python -m dejasolve.ingest --compare` with the local model reachable
