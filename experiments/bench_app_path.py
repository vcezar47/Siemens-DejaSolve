...
`benchmarks/bench.py` measures each warm-start construction on its own: no verifier, no
...
`pipeline.analyse` runs. The app shortlists the k nearest, lets the verifier
...
the same two calls `analyse` makes -- over `benchmarks/bench.py`'s own 200 query cases
(seed 99), so the headline the deck quotes from `results.json` can be checked
against what a person clicking Analyse would see.

    python -m experiments.bench_app_path

Writes bench_app_path_results.json. No new physics, no new archive: every
number here is downstream of the same model and the same cases.jsonl benchmarks/bench.py
uses.
"""
...
from dejasolve import pipeline
from dejasolve import model
from dejasolve import sweep

ARCHIVE = Path("data/archive/cases.jsonl")
N_QUERIES = 200
SEED = 99


def run(archive_path: Path = ARCHIVE, n: int = N_QUERIES, seed: int = SEED,
        out: Path | None = Path("results/bench_app_path_results.json")) -> dict:
    arc = pipeline.Archive(archive_path)
    ...
    payload = {
        ...
        "what": "pipeline.Archive.select + warm_start over benchmarks/bench.py's 200 "
...
def report(v: dict) -> None:
    ...
    print(f"  reduction vs nominal       "
          ...
          f"(benchmarks/bench.py 'ranked + 2nd order' arm: 57.2%)")
