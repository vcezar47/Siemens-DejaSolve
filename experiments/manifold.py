...
`experiments/geometry.py` answered "what if the pipes were real?" on a two-branch circuit:
...
excavator's boom, arm, bucket and swing hang off one pump exactly like this.
    ...
    python -m experiments.manifold --selftest        # Jacobian and dF/dp, every width
    python -m experiments.manifold                   # the scaling study
...
from experiments.geometry import (DP_REG_LINE, M_TURB, d_line_flow, line_flow,
...
#: fixed once a machine is built. `benchmarks/wide_sweep.py` established this split on the
...
def study_width(nb: int, n_archive: int, n_query: int) -> dict:
    ...
    for p in c.sample_cases(n_query, seed=99):
        ...
        # error -- the same signal benchmarks/bench.py ranks on, at this width
        cand = np.argsort(d)[:K_RANKED]
        best, best_err = int(cand[0]), np.inf
        for cc in cand:
            if sens[cc] is None:
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=Path("results/manifold_results.json"))
    args = ap.parse_args()
