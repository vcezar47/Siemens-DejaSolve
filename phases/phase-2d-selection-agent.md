...
`python agent_select.py` (ceiling, floor and physics arms, no GPU) ·
`python agent_select.py --model granite4` (the model arm).

`topk.py` found that walking deeper into the archive *by distance* does not pay. That is a statement about the
