...
#: `benchmarks/selftest.py`, and by the phase-1 summary hash, which must not move.
HARDWARE = {
...
#: private copy inside `benchmarks/wide_sweep.py`, which needed it to generate synthetic
...
#: wrong way to share it. `benchmarks/wide_sweep.py` now imports this one.
HARDWARE_SPREAD = {
    "cd": (0.85, 1.15),
    "R_leak": (0.7, 1.4),
...
def hardware(p: dict) -> dict:
    ...
    ``D_mot`` stays a valid shorthand for "both motors", because `benchmarks/fold.py`
...
def motor_constants(p: dict, shaft: str | None = None) -> tuple[float, float]:
    ...
    `shaft` argument, so `benchmarks/fold.py`'s use of it (a variant circuit built
...
def dresidual_dp(x, p: dict) -> np.ndarray:
    ...
    be trustworthy enough to ship inside a Case Card. `benchmarks/selftest.py` checks it
...
# already validated by benchmarks/selftest.py:
#
...
def jacobian_fd(x, p: dict, F0, step: float = FD_STEP) -> np.ndarray:
    ...
    wrong in exactly the region where it needs to be right. ``benchmarks/bench.py``
