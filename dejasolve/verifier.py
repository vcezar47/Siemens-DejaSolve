...
#: the hard way: `benchmarks/wide_sweep.py --tolerance 0.02` refuses **200 of 200**
#: transfers, including every same-machine one, which is a gate nobody would
#: keep switched on.
#:
#: 5% is a stated assumption rather than a tuned constant, and `benchmarks/wide_sweep.py`
...
#: `benchmarks/dimensionality.py` finds in the coverage rule, caught here before shipping.
...
def estimate_regime(p: dict) -> dict:
    ...
    `benchmarks/agent_select.py`'s physics ranker reads, and changing what these three
