...
#: how many candidates the `ranked` arm scores. Five because `agent_select.py`
...
def sensitivities(records: list[dict]) -> list[np.ndarray | None]:
    """The per-card solution tangents, as written by `sweep.py`.
...
def hessians(records: list[dict]) -> list[np.ndarray | None]:
    """The per-card solution Hessians (curvature tensors), as written by `sweep.py`."""
    out: list[np.ndarray | None] = []
    for r in records:
        h = r.get("hessian")
