...
of drifting apart.
    ...
    python -m dejasolve data/logs/note-ro.txt
...
#: different a case is, not how far its answer will have to move. `benchmarks/bench.py`
...
class Archive:
    ...
    reads them -- `benchmarks/failure_zone.py` measured whether proximity to a failure
    ...
    def __init__(self, path: Path):
        if not path.exists():
            raise FileNotFoundError(f"no archive at {path} -- run `python -m dejasolve.sweep`")
        ...
        #: before `dejasolve/sweep.py` recorded curvature. `transfer_start_second_order`
    ...
    def nearest_failure(self, params: dict) -> dict | None:
        ...
        failures in 400 and `benchmarks/failure_zone.py` calls that underpowered. A
...
def analyse(text: str, name: str, archive: Archive,
    ...
    # the same comparison benchmarks/bench.py does: a nominal guess built from this card's
    ...
    # the page can only ever draw the fixture `benchmarks/viz.py` shipped, and the case the
