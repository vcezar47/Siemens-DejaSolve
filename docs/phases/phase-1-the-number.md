...
residual → ✅ [`../figs/convergence.png`](../../results/figs/convergence.png)
...
| file | role |
|---|---|
| [`../model.py`](../../dejasolve/model.py) | The physics and the damped-Newton initialisation solve |
| [`../sweep.py`](../../dejasolve/sweep.py) | 400-case Latin-hypercube sweep → the archive |
| [`../bench.py`](../../benchmarks/bench.py) | Cold vs retrieval-warm on 200 fresh cases → `results.json` |
| [`../plot_convergence.py`](../../benchmarks/plot_convergence.py) | The gate figure, drawn from `results.json` only |
...
### Exemplars picked automatically by `benchmarks/bench.py`
...
assumption — `benchmarks/bench.py` runs the sensitivity every time:
...
agreement holds to 3e-08 bar, same as every other arm. `benchmarks/selftest.py` gained a
