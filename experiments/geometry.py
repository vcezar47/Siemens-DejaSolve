...
`dejasolve/static/index.html` carries a disclaimer: *"the layout is illustrative — the
...
plumbing?"**
    ...
    python -m experiments.geometry --selftest     # the Jacobian, against central differences
    python -m experiments.geometry                # sweep, then warm-start on fresh cases

Nothing here is imported by `run_all.py`, `benchmarks/bench.py` or `dejasolve/model.py`. The frozen
...
def check_jacobian(n_cases: int = 40, seed: int = 7, tol: float = 1e-5) -> int:
    """Same discipline as `benchmarks/selftest.py`: this circuit's Jacobian is hand-derived
    ...
    def ladder(top: float) -> list[float]:
        ...
        Sampled this way rather than independently for the reason `benchmarks/selftest.py`
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=Path("results/geometry_results.json"))
    args = ap.parse_args()
