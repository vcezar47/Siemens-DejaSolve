"""One-off: load archive/cases.jsonl and archive/failures.jsonl into Aurora.

Populates the `cases` table archive_aurora.AuroraArchive reads from. Run once
after the data.yaml stack exists, and again after any `python sweep.py`
...
with the command overridden to `python sync_archive_to_aurora.py` is the
simplest way from the console -- see docs/aws-deployment-plan.md), not from a
laptop.

    python sync_archive_to_aurora.py
"""
...
def main() -> None:
    ...
    try:
        ...
        solved = _read_jsonl(ROOT / "archive" / "cases.jsonl")
        failed = _read_jsonl(ROOT / "archive" / "failures.jsonl")
        if not solved:
            raise SystemExit(f"no records at {ROOT / 'archive' / 'cases.jsonl'} "
                              "-- run `python sweep.py` first")
