...
`python selftest.py` · `python wide_sweep.py` · both inside `python run_all.py`.
...
bit-identically to before. Two independent detectors say so: `selftest.py` measures the worst state difference
...
## selftest.py — built before the change, not after
...
## wide_sweep.py — and the realistic archive is not 400 unique machines
...
threshold degrades as the parameter count grows**, which is the same failure mode `dimensionality.py` finds in
...
- **Promoting the constants into `PARAM_BOUNDS` and the main sweep.** That would move `results.json` and the
  frozen hash for a result that is not the headline. `wide_sweep.py` is a separate experiment, like `fold.py`
  and `surrogate.py`.
