...
def sweep_fold(n: int, seed: int) -> tuple[list[dict], list[dict], dict]:
    ...
    in `benchmarks/failure_zone.py`; it is not assumed here, and nothing in the warm-start
    ...
    for i, p in enumerate(fold_cases(n, seed)):
        ...
        kept.append({
            ...
            #: the same tangent `dejasolve/sweep.py` records on the base circuit. It
...
def main() -> None:
    ...
    ap.add_argument("--out", type=Path, default=Path("results/fold_results.json"))
    ap.add_argument("--fig", type=Path, default=Path("results/figs/verifier.png"))
    args = ap.parse_args()
