...
**Built:** Tue 25 Aug 2026 · [`../coupled.py`](../../experiments/coupled.py) ·
[`../manifold.py`](../../experiments/manifold.py) · not in `run_all.py`
...
**1. A wider machine** (`experiments/manifold.py`, up to 42 unknowns / 114 parameters).
...
**2. A load-sensing pump** (`experiments/coupled.py`, first version). The pump reads the
...
**3. A shared return manifold** (`experiments/coupled.py`, current). Every branch exhausts
...
1. **Add the decomposed arm to `benchmarks/bench.py` as a fourth baseline** and report it.
