...
[`../figs/verifier.png`](../../results/figs/verifier.png)
...
## The rules ([`../verifier.py`](../../dejasolve/verifier.py))
...
| file | role |
|---|---|
| [`../verifier.py`](../../dejasolve/verifier.py) | The rules, the cheap regime estimators, both gates |
| [`../fold.py`](../../benchmarks/fold.py) | The fold-circuit variant, multi-start root finding, naive vs verified experiment, the figure |
...
side by side in `benchmarks/fold.py` and their answers are diffed, because the fallback is
...
number is produced by `benchmarks/fold.py`, which never had an override and still does not:
...
**What it cost to demonstrate:** one new fixture, `data/logs/run-bigpump.log`, a 118
