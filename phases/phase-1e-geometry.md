...
[`../geometry.py`](../geometry.py) · `python geometry.py --retrieval`

Not in `run_all.py` and not imported by `model.py`, `bench.py` or anything that
...
the same treatment `selftest.py` gives the base circuit — central differences,
...
`selftest.py`'s own docstring warns about.
...
the way `agent_select.py` defines it.*
