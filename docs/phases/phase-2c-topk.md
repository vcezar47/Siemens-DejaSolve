...
`benchmarks/fold.py` does. Every extra candidate after that costs solves: 7.89 → 8.60 mean iterations, a 9% bill for an
improvement of zero.
...
That is where a selection agent would earn its place, and `experiments/topk.py` is the harness it would be measured in:
