...
Panels:
  A  one case, residual per Newton iteration, cold vs warm — same final
     residual, fewer steps
...
def draw(results: dict, out: Path) -> None:
    ...
    ax1.set_title(f"A · one case ({ex['case_id']})\nsame equations, same tolerance, "
                  "different starting guess", fontsize=10)
    ax1.legend(fontsize=9)
    ax1.grid(alpha=0.3)

    # --- B: distribution over all query cases -----------------------------
    ci = np.array([c["cold"]["iterations"] for c in results["cases"]
                   if c["cold"]["converged"] and c["warm"]["converged"]])
    wi = np.array([c["warm"]["iterations"] for c in results["cases"]
                   if c["cold"]["converged"] and c["warm"]["converged"]])
    bins = np.arange(0, max(ci.max(), wi.max()) + 2) - 0.5
    ax2.hist(ci, bins=bins, alpha=0.65, color=COLD, label=f"cold (mean {ci.mean():.1f})")
    ax2.hist(wi, bins=bins, alpha=0.65, color=WARM, label=f"warm (mean {wi.mean():.1f})")
    ax2.set_xlabel("Newton iterations to converge")
    ax2.set_ylabel("cases")
    ax2.set_title(f"B · {len(ci)} fresh cases vs an archive of "
                  f"{cfg['archive_size']}\n{s['iteration_reduction_pct']:.0f}% fewer "
                  "iterations in total", fontsize=10)
    ...
    if rows:
        ...
        ax3.barh(y - 0.2, [r["cold_failures"] for r in rows], height=0.38,
                 color=COLD, label="cold start")
        ax3.barh(y + 0.2, [r["warm_failures"] for r in rows], height=0.38,
        ...
        ax3.set_title("C · the worse the solver's Jacobian,\nthe more the archive is worth",
                      fontsize=10)
